(function(){
let currentBookType=null,currentEpubBook=null,currentRendition=null,currentPdfDoc=null,currentPdfTotalPages=0,currentPdfPageNum=1,currentTxtRaw=null,currentTxtPages=[],currentTxtPageIndex=0,smartChapterMode=!1,currentTxtChunks=[],currentChapterIndex=0,currentFileName="",currentFontSize=100,currentTheme="light",currentBookUrlOrId="";
const readerContainer=document.getElementById('readerContainer');
let readerArea=document.getElementById('readerArea');
const pageSlider=document.getElementById('pageSlider'),fileInput=document.getElementById('fileInput'),searchInput=document.getElementById('searchInput'),clearSearchBtn=document.getElementById('clearSearchBtn'),searchDropdown=document.getElementById('searchDropdown'),localMatchList=document.getElementById('localMatchList'),globalMatchList=document.getElementById('globalMatchList'),toolbarOverlay=document.getElementById('toolbarOverlay'),sidebar=document.getElementById('sidebar'),sidebarMask=document.getElementById('sidebarMask'),tocListEl=document.getElementById('tocList'),bookTitleEl=document.getElementById('bookTitle'),themeToggleBtn=document.getElementById('themeToggleBtn');
let currentSearchTerm="",currentSearchMatches=[],searchDebounceTimer,isAnimating=!1,lastTapTime=0;
let db=null;
const DB_NAME="ZZXMobileDB",STORE_NAME="books";
function initDB(){return new Promise((resolve,reject)=>{const req=indexedDB.open(DB_NAME,1);req.onerror=()=>reject(req.error);req.onsuccess=()=>{db=req.result;resolve(db);};req.onupgradeneeded=e=>{const dbr=e.target.result;if(!dbr.objectStoreNames.contains(STORE_NAME))dbr.createObjectStore(STORE_NAME,{keyPath:"id"})}})}
async function saveBook(id,blob,name,type){if(!db)await initDB();const tx=db.transaction([STORE_NAME],"readwrite");tx.objectStore(STORE_NAME).put({id,blob,fileName:name,fileType:type,timestamp:Date.now()})}
async function loadBook(id){if(!db)await initDB();return new Promise(resolve=>{const tx=db.transaction([STORE_NAME],"readonly"),req=tx.objectStore(STORE_NAME).get(id);req.onsuccess=()=>resolve(req.result)})}
...
readerContainer.addEventListener('click',e=>{if(toolbarOverlay.style.display==='block'&&!e.target.closest('.toolbar-overlay'))closeToolbar()});
sidebarMask.addEventListener('click',closeSidebar);
document.getElementById('closeSidebarBtn').addEventListener('click',closeSidebar);
document.getElementById('tapCenter').addEventListener('click',e=>{e.stopPropagation();toggleToolbar()});
...
fileInput.addEventListener('change',async e=>{if(e.target.files.length){await processFile(e.target.files[0]);fileInput.value=''}});
document.getElementById('urlLoadBtn').addEventListener('click',()=>{const url=prompt('输入图书URL:');if(url)loadFromUrl(url).catch(err=>alert('加载失败: '+err.message))});
document.getElementById('fontMinusBtn').addEventListener('click',()=>{adjustFontSize(-10);if(currentBookType==='txt')renderTxtPages()});
document.getElementById('fontPlusBtn').addEventListener('click',()=>{adjustFontSize(10);if(currentBookType==='txt')renderTxtPages()});
themeToggleBtn.addEventListener('click',()=>setTheme(currentTheme==='light'?'dark':'light'));
document.getElementById('smartChapterBtn').addEventListener('click',()=>{if(currentBookType!=='txt')return;smartChapterMode=!smartChapterMode;localStorage.setItem(`txt_smart_mode_${currentFileName}`,smartChapterMode);loadTxtWithSmartChapter()});
document.getElementById('tocBtn').addEventListener('click',()=>{openSidebar();closeToolbar()});
async function processFile(file){if(!file)return;const name=file.name,ext=name.split('.').pop().toLowerCase();const buffer=await file.arrayBuffer();currentBookUrlOrId=`file_${name}_${Date.now()}`;await saveBook(currentBookUrlOrId,new Blob([buffer]),name,ext);currentFileName=name;clearSearch();if(ext==='epub')await loadEpub(buffer,name);else if(ext==='pdf')await loadPdf(buffer,name);else if(ext==='txt')await loadTxt(buffer,name);closeToolbar();saveConfig()}
async function loadFromUrl(url){const resp=await fetch(url);if(!resp.ok)throw new Error(`HTTP ${resp.status}`);const blob=await resp.blob();const filename=url.split('/').pop()||"book";await processFile(new File([blob],filename))}
async function loadEpub(buffer,filename){clearReader();currentBookType='epub';const blob=new Blob([buffer],{type:'application/epub+zip'});currentEpubBook=ePub(URL.createObjectURL(blob));await setupEpubFlip();currentRendition=currentEpubBook.renderTo("readerArea",{width:"100%",height:"100%",spread:"none",flow:"paginated"});await currentRendition.display();currentRendition.themes.register('light',{body:{background:'#fefefe',color:'#1e293b'}});currentRendition.themes.register('dark',{body:{background:'#11131f',color:'#e2e8f0'}});setTheme(currentTheme);currentRendition.themes.fontSize(currentFontSize+"%");const nav=await currentEpubBook.loaded.navigation;buildTocFromEpub(nav.toc);currentRendition.on('relocated',()=>{});bookTitleEl.innerText=filename;await loadProgress()}
function buildTocFromEpub(toc){tocListEl.innerHTML='';const renderItems=(items,parent)=>{items.forEach(item=>{const li=document.createElement('li');li.className='toc-item';li.innerText=item.label||'章节';if(item.href)li.addEventListener('click',()=>{currentRendition.display(item.href);closeSidebar()});parent.appendChild(li);if(item.subitems)renderItems(item.subitems,parent)})};renderItems(toc,tocListEl)}
async function loadPdf(buffer,filename){clearReader();currentBookType='pdf';currentPdfDoc=await pdfjsLib.getDocument({data:new Uint8Array(buffer)}).promise;currentPdfTotalPages=currentPdfDoc.numPages;currentPdfPageNum=1;await renderPdfForFlip();buildPdfToc();bookTitleEl.innerText=filename;await loadProgress()}
function buildPdfToc(){tocListEl.innerHTML='';for(let i=1;i<=currentPdfTotalPages;i++){const li=document.createElement('li');li.className='toc-item';li.innerText=`第${i}页`;li.addEventListener('click',()=>{goToPage(i-1);closeSidebar()});tocListEl.appendChild(li)}}
...
function splitIntelligentChapters(text){const unitCounter={},pattern=/^第([\d零一二三四五六七八九十百千万]+)([章节卷回部篇集辑课程])/gm;let match;while((match=pattern.exec(text))!==null){const unit=match[2];unitCounter[unit]=(unitCounter[unit]||0)+1}let bestUnit=null,maxCount=0;for(let u in unitCounter)if(unitCounter[u]>maxCount){maxCount=unitCounter[u];bestUnit=u}let splitPattern;if(bestUnit){splitPattern=new RegExp(`^(第[\\d零一二三四五六七八九十百千万]+${bestUnit})`,'gm')}else{splitPattern=/^(第[\d零一二三四五六七八九十百千万]+[章节卷回部篇集辑课程]?)/gm}const lines=text.split(/\r?\n/),chapters=[];let currentTitle="序言",currentContent=[];for(let line of lines){const trimmed=line.trim();splitPattern.lastIndex=0;if(splitPattern.test(trimmed)&&trimmed.length<50){if(currentContent.length)chapters.push({title:currentTitle,content:currentContent.join('\n')});currentTitle=trimmed;currentContent=[]}else currentContent.push(line)}if(currentContent.length)chapters.push({title:currentTitle,content:currentContent.join('\n')});if(chapters.length===0)chapters=[{title:"全文",content:text}];return chapters}
function escapeHtml(s){return s.replace(/[&<>]/g,c=>c==='&'?'&amp;':c==='<'?'&lt;':'&gt;')}
function clearSearch(){currentSearchTerm="";searchInput.value="";searchDropdown.style.display="none";removeHighlights();localMatchList.innerHTML="";globalMatchList.innerHTML=""}
function performSearch(query){currentSearchTerm=query;if(!query.trim()){searchDropdown.style.display="none";removeHighlights();return}const lowerQuery=query.toLowerCase(),localText=getCurrentVisibleText(),localMatches=[];let idx=localText.toLowerCase().indexOf(lowerQuery);while(idx!==-1){const start=Math.max(0,idx-20),end=Math.min(localText.length,idx+query.length+20);localMatches.push({start:idx,end:idx+query.length,text:localText.slice(start,end).replace(/\n/g,' ')});idx=localText.toLowerCase().indexOf(lowerQuery,idx+1)}currentSearchMatches=localMatches;localMatchList.innerHTML=localMatches.length?localMatches.map(m=>`<li>...${escapeHtml(m.text)}...</li>`).join(''):'<li>无匹配</li>';const globalRes=[];if(currentBookType==='txt'&&smartChapterMode&&currentTxtChunks.length){currentTxtChunks.forEach((ch,i)=>{const count=(ch.content.toLowerCase().split(lowerQuery).length-1);if(count>0)globalRes.push({chapterIndex:i,title:ch.title,count})})}globalMatchList.innerHTML=globalRes.length?globalRes.map(r=>`<li data-chapter="${r.chapterIndex}">${escapeHtml(r.title)} (${r.count})</li>`).join(''):'<li>全文无匹配</li>';searchDropdown.style.display="block";highlightLocalMatches()}
function getCurrentVisibleText(){if(currentBookType==='txt'){if(smartChapterMode&&currentTxtPages.length)return currentTxtPages[currentTxtPageIndex]||'';return currentTxtRaw||''}return''}
function highlightLocalMatches(){removeHighlights();if(!currentSearchTerm)return;const container=document.querySelector('.txt-viewer')||document.querySelector('.reader-inner.active .txt-viewer');if(container){const regex=new RegExp(`(${escapeRegex(currentSearchTerm)})`,'gi');container.innerHTML=container.innerHTML.replace(regex,'<mark>$1</mark>')}}
function removeHighlights(){const container=document.querySelector('.txt-viewer')||document.querySelector('.reader-inner.active .txt-viewer');if(container)container.innerHTML=container.innerHTML.replace(/<\/?mark[^>]*>/gi,'')}
...
localMatchList.addEventListener('click',e=>{const li=e.target.closest('li');if(!li||!currentSearchMatches.length)return;const index=Array.from(localMatchList.children).indexOf(li),match=currentSearchMatches[index];if(!match)return;const container=document.querySelector('.txt-viewer')||document.querySelector('.reader-inner.active .txt-viewer');if(container){const walker=document.createTreeWalker(container,NodeFilter.SHOW_TEXT);let node,offset=0;while((node=walker.nextNode())){const len=node.textContent.length;if(offset+len>match.start){const range=document.createRange();range.setStart(node,match.start-offset);range.setEnd(node,match.end-offset);range.startContainer.parentElement.scrollIntoView({behavior:'smooth',block:'center'});break}offset+=len}}});
globalMatchList.addEventListener('click',e=>{const li=e.target.closest('li');if(!li)return;const chapterIdx=parseInt(li.dataset.chapter,10);if(!isNaN(chapterIdx)&&currentTxtChunks.length){currentChapterIndex=chapterIdx;currentTxtRaw=currentTxtChunks[chapterIdx].content;renderTxtPages();closeToolbar();setTimeout(()=>performSearch(currentSearchTerm),300)}});
